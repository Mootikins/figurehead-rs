...
fn test_td_layer_nodes_keep_natural_heights() {
    ...
    db.add_shaped_node("A", "Rectangle", NodeShape::Rectangle)
        .unwrap();
    db.add_shaped_node("B", "Diamond", NodeShape::Diamond)
        .unwrap();
    ...
    assert_eq!(
        node_b.height, 3,
        "Diamond (Box style) should have natural height 3"
    );
}
...
fn test_split_edges_are_grouped() {
    ...
    let edges_from_d: Vec<_> = result.edges.iter().filter(|e| e.from_id == "D").collect();

    assert_eq!(edges_from_d.len(), 2);

    for edge in &edges_from_d {
        assert!(
            edge.junction.is_some(),
            "Edge to {} should have junction",
            edge.to_id
        );
    ...
    let indices: Vec<_> = edges_from_d.iter().filter_map(|e| e.group_index).collect();
